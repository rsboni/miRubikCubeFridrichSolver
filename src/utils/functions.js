...
export const calculateResultantAngle = (
  alpha,
  roationVector,
  ...
  const gamaInverse =
    ...
    sin(alpha / 2) * sin(beta / 2) * (lx * mx + ly * my + lz * mz);
  const gama =
    2 *
    toDegrees(
      Math.acos(
        Math.abs(gamaInverse) > 1
          ? gamaInverse / Math.abs(gamaInverse)
          : gamaInverse
      )
    );
  ...
  return {
    gama,
    rotationVector: [
      isNaN(nx) || !isFinite(nx) ? 1 : nx,
      isNaN(ny) || !isFinite(nx) ? 1 : ny,
      isNaN(nz) || !isFinite(nx) ? 1 : nz,
    ],
  };
};

export const getCubePositionDiffrence = (
  movedPosition,
  currentPosition,
  xMove,
  yMove
